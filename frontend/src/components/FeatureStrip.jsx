...
function FeatureItems({ className = "" }) {
  return (
...
export default function FeatureStrip({ embedded = false }) {
  if (embedded) {
    return (
      <div className="mb-6" aria-label="Service highlights">
        <FeatureItems className="grid grid-cols-1 sm:grid-cols-2 gap-3" />
      </div>
    );
  }
  ...
  return (
    <div className="bg-white border-t border-slate-200" aria-label="Service highlights">
      <FeatureItems className="max-w-[1280px] mx-auto px-4 sm:px-6 py-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4" />
    </div>
  );
}
